...
const char PANTHEIOS_FE_PROCESS_IDENTITY[] = "test.unit.fe.WindowsRegistry";
...
namespace
{
    static const char ROOT_KEY_PATH[] = "SOFTWARE\\Synesis Software\\Logging Tools\\Pantheios\\fe.WindowsRegistry";
    ...
    static void ensure_HKCU(int debugLevel, int releaseLevel, int starLevel)
    {
        ...
        if(debugLevel >= 0)
        {
            key.set_value("Debug", DWORD(debugLevel));
        }
        if(releaseLevel >= 0)
        {
            key.set_value("Release", DWORD(releaseLevel));
        }
        if(starLevel >= 0)
        {
            key.set_value("*", DWORD(starLevel));
        }
    }

    static void ensure_HKLM(int debugLevel, int releaseLevel, int starLevel)
    {
