...
PARAM_RANGE.each do |i|
		...
		f << "".ljust(len0) + ", pan_char_t const* p#{i - 1}, int l#{i - 1}); /* -1 => 'strlen(p#{i - 1})' */" << ENDL
		f << "" << ENDL
...
PARAM_RANGE.each do |i|
		...
		f << "{" << ENDL
		f << "  const pan_slice_t slices[#{i}] =" << ENDL
		f << "  {" << ENDL
		...
		f << "  };" << ENDL
		f << "" << ENDL
		f << "  return pantheios_log_n(severity, STLSOFT_NUM_ELEMENTS(slices), slices);" << ENDL
		f << "}" << ENDL
		f << "" << ENDL
...
PARAM_RANGE.each do |i|
		...
		f << "{" << ENDL
		f << "  if(!pantheios_isSeverityLogged(severity))" << ENDL
		f << "  {" << ENDL
		f << "    return 0;" << ENDL
		f << "  }" << ENDL
		f << "  else" << ENDL
		...
		if INITIALISERS_INLINE
			...
			f << "        pan_slice_t(l0, p0)" << ENDL
		end
		(1 ... i). each do |j|
			if INITIALISERS_INLINE
				f << ", pan_slice_t(l#{j}, p#{j})"
			else
				f << "      , pan_slice_t(l#{j}, p#{j})" << ENDL
			end
		end
		if INITIALISERS_INLINE
			f << "" << ENDL
...
PARAM_RANGE.each do |i|
		...
		f << "".ljust(len0) + ", pan_slice_t const& s#{i - 1});" << ENDL
...
PARAM_RANGE.each do |i|
		...
		f << "".ljust(len0) + ", size_t l#{i - 1}, pan_char_t const* p#{i - 1})" << ENDL
		f << "{" << ENDL
		...
		f << "".ljust(len0) + ", pan_slice_t(p#{i - 1}, l#{i - 1}));" << ENDL
		f << "}" << ENDL
		f << "" << ENDL
...
PARAM_RANGE.each do |i|
		...
		if USE_USING_DECLARATION
				f << "    PANTHEIOS_DECLARE_SHIM_PAIR_();" << ENDL
				f << ENDL
		end

		f << "    return internal::log_dispatch_#{i}(severity" << ENDL
		(0 ... i - 1).each do |j| 
		...
		if USE_SHIM_PAIR_MACRO
				f << "#{CALL_LMARGIN}, PANTHEIOS_INVOKE_SHIM_PAIR_(v#{i - 1}));" << ENDL
		elsif USE_USING_DECLARATION
				f << "#{CALL_LMARGIN}, c_str_len_a(v#{i - 1}), c_str_data_a(v#{i - 1}));" << ENDL
		else
				f << "#{CALL_LMARGIN}, ::stlsoft::c_str_len_a(v#{i - 1}), ::stlsoft::c_str_data_a(v#{i - 1}));" << ENDL
		end
		f << "  }" << ENDL
		f << "}" << ENDL
		f << "" << ENDL
...
SEVERITY_LEVELS.each do |severityLevel|
	...
	PARAM_RANGE.each do |i|
		...
		f << "  if(!pantheios_isSeverityLogged(PANTHEIOS_SEV_#{severityLevel}))" << ENDL
		f << "  {" << ENDL
		f << "    return 0;" << ENDL
		f << "  }" << ENDL
		f << "  else" << ENDL
		...
		f << "    return internal::log_dispatch_#{i}(PANTHEIOS_SEV_#{severityLevel}" << ENDL
